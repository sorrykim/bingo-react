...
import {takeTurn as player2TakeTurn, boardSelector as player2BoardSelector} from './player2';
import {sortBingos} from '../models/bingo';

// action types
const TAKE_TURN = 'bingo/player1/TAKE_TURN';
const MARK_CELL = 'bingo/player1/MARK_CELL';
const ADD_BINGOS = 'bingo/player1/ADD_BINGOS';
const WIN = 'bingos/player1/WIN';
const LEAVE_TURN = 'bingo/player1/LEAVE_TURN';
const RESET = 'bingo/player1/RESET';

export function isTurnSelector(state) {
  return state.player1.isTurn;
}

export function boardSelector(state) {
  return state.player1.board;
}

export function bingosSelector(state) {
  return state.player1.bingos;
}
...
function leaveTurnEpic(action$, state$) {
  return action$.pipe(
    ...
    map(() => {
      ...
      isBingo || isPlayer2Bingo ? actions.push(endGame()) : actions.push(player2TakeTurn());
      return actions;
    })
  );
}
...
function markCellEpic(action$, state$) {
  action$.pipe(
    ...
    map(({number}) => {
      ...
      const player2Bingos = findNewBingos(player2BoardSelector(state$.value), number);
